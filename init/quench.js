const sampleFoodItems = [
    {
        name: "Classic Lemonade",
        description: "Refreshing lemonade made with freshly squeezed lemons and a hint of mint.",
        image: {
            url: "https://images.healthshots.com/healthshots/en/uploads/2023/04/15092339/lemon-juice-1600x900.jpg"
        },
        price: 60
    },
    {
        name: "Mango Smoothie",
        description: "A creamy blend of ripe mangoes, yogurt, and a touch of honey.",
        image: {
            url: "https://www.indianhealthyrecipes.com/wp-content/uploads/2021/04/mango-milkshake-recipe.jpg"
        },
        price: 90
    },
    {
        name: "Iced Coffee",
        description: "Chilled coffee served with milk and a hint of vanilla, topped with whipped cream.",
        image: {
            url: "https://frostingandfettuccine.com/wp-content/uploads/2022/12/Caramel-Iced-Coffee-6.jpg"
        },
        price: 70
    },
    {
        name: "Berry Iced Tea",
        description: "Refreshing iced tea infused with mixed berries and a splash of lemon.",
        image: {
            url: "https://waystomyheart.com/wp-content/uploads/2019/05/ways-to-my-heart-iced-tea.jpg"
        },
        price: 65
    },
    {
        name: "Fruit Punch",
        description: "A tropical blend of fruit juices served over ice, perfect for a hot day.",
        image: {
            url: "https://heatherchristo.com/wp-content/uploads/2019/05/47755684632_9b1a926524_o.jpg"
        },
        price: 80
    },
    {
        name: "Chocolate Milkshake",
        description: "Thick and creamy chocolate milkshake topped with whipped cream and chocolate syrup.",
        image: {
            url: "https://www.sharmispassions.com/wp-content/uploads/2012/07/chocolate-milkshake1.jpg"
        },
        price: 100
    }
];

module.exports = { data: sampleFoodItems };